CREATE PROCEDURE [dbo].[usp_DOR_SCHEDULE_MANAGE]  
 @xml XML,          
...
BEGIN  
 ...
 WHEN MATCHED AND (isnull(src.inserted_by,'') <> 'AppleCore') THEN           
   ...
   trg.qty = (case when src.deleted_by is not null then 
				CASE WHEN @from_schedule_production = 1 AND trg.status_id <> 1 THEN trg.qty ELSE src.qty END 
				else src.qty end)   ,          
   
   trg.act_qty = (CASE WHEN @from_schedule_production = 1 
					  THEN CASE WHEN trg.status_id= 1 THEN src.qty ELSE trg.act_qty END 
					  ELSE src.act_qty 
					  END) ,       
  ...
  trg.seq_no_prod = (case when @from_schedule_production = 1 then   
		   case when trg.line_facility_id <>  src.line_facility_id AND trg.status_id= 1  then ([dbo].[fn_GetMaxSeqNumberForvarient](src.schedule_date,src.line_facility_id,null
		   ,@from_schedule_production))  
  ...
  trg.nitemseqproduction = (case when @from_schedule_production = 1 then   
		   case when trg.line_facility_id <>  src.line_facility_id AND trg.status_id= 1  then ([dbo].[fn_GetMaxSeqNumberForvarient](src.schedule_date,src.line_facility_id,src.item_id
		   ,@from_schedule_production))  
  ...
  trg.production_start_time = (CASE WHEN @from_schedule_production = 1 
								THEN CASE WHEN trg.status_id= 1  THEN src.production_start_time ELSE trg.production_start_time END
								ELSE src.production_start_time END),        

  trg.production_end_time = (CASE WHEN @from_schedule_production = 1 
								THEN CASE WHEN trg.status_id= 1  THEN src.production_end_time ELSE trg.production_end_time END
								ELSE src.production_end_time END),
    
  trg.production_line_facility_id =   (CASE WHEN @from_schedule_production = 1 
								THEN CASE WHEN trg.status_id= 1  THEN src.line_facility_id ELSE trg.production_line_facility_id END
  ...
  trg.ncrewproduction =   (CASE WHEN @from_schedule_production = 1 
								THEN CASE WHEN trg.status_id= 1  THEN src.crew ELSE trg.ncrewproduction END
								ELSE src.crew END)  ,  
  trg.nspeedproduction =    (CASE WHEN @from_schedule_production = 1 
								THEN CASE WHEN trg.status_id= 1  THEN convert(numeric(18,2),src.t_max/60) ELSE trg.nspeedproduction END
								ELSE convert(numeric(18,2),src.t_max/60) END) ,
  
  trg.nlinehrsproduction =  (CASE WHEN @from_schedule_production = 1 
								THEN CASE WHEN trg.status_id= 1  THEN (CASE WHEN src.crew IS NOT NULL AND isnull(src.t_max,0) > 0 AND isnull(src.per,0) > 0 THEN CAST(((src.act_qty/src.t_max) * (1.0/src.per)) * 100 
  ...
  trg.re_run_production = (CASE WHEN @from_schedule_production = 1 
								THEN CASE WHEN trg.status_id= 1  THEN src.re_run_units ELSE trg.re_run_production END
								ELSE src.re_run_units END),  
  trg.over_run_production =  (CASE WHEN @from_schedule_production = 1 
								THEN CASE WHEN trg.status_id= 1  THEN src.over_run_units ELSE trg.over_run_production END
								ELSE src.over_run_units END),  
  trg.break_use_production = (CASE WHEN @from_schedule_production = 1 
								THEN CASE WHEN trg.status_id= 1  THEN src.use_break ELSE trg.break_use_production END
								ELSE src.use_break END),  
  trg.lunch_use_production = (CASE WHEN @from_schedule_production = 1 
								THEN CASE WHEN trg.status_id= 1  THEN src.use_lunch ELSE trg.lunch_use_production END
								ELSE src.use_lunch END),  
  trg.crew_size_actual_production = (CASE WHEN @from_schedule_production = 1 
								THEN CASE WHEN trg.status_id= 1  THEN src.crew_size_actual ELSE trg.crew_size_actual_production END
...
SELECT  top 1 @deleted_by = a.deleted_by  ,@IS_Delete = (case when a.deleted_by is not null then 
						CASE WHEN @from_schedule_production = 1 AND b.status_id <> 1  AND b.act_qty IS NOT NULL THEN 1 ELSE 0 END 
