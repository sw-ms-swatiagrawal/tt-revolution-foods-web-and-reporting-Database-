CREATE PROCEDURE [dbo].[usp_DOR_SCHEDULE_MANAGE_LOAD_APPLECORE_SCHEDULE_backup_14_06_2018]
 @xml XML,        
...
BEGIN
 ...
 @status_id INT = (select status_id from dbo.RF_DWH_STATUS where status_desc='Not Completed');        
   ...
   SELECT schedule_id,b.line_facility_id,shift_id,a.item_id,per,qty,         
	...
	--ELSE 
		CASE WHEN isnull([dbo].[fn_GetMaxSeqNumber](schedule_date,b.line_facility_id,a.item_id,a.inserted_by),0) = 0
			THEN  isnull([dbo].[fn_GetMaxSeqNumber](schedule_date,null,null,a.inserted_by),0) + (dense_rank() OVER(partition by schedule_date  ORDER BY schedule_date,a.line_facility_id,im.item_nm))-- im.item_nm,vm.variation_nm))  --,item_id))
...
,re_run_production,over_run_production,break_use_production,lunch_use_production,crew_size_actual_planing,crew_size_actual_production,status_id)
VALUES(src.line_facility_id,src.shift_id,src.item_id,src.per,src.qty,src.schedule_date,src.inserted_date,src.inserted_by,--@next_seq_no,
	...
	src.re_run_units,src.over_run_units,src.use_break,src.use_lunch,
	src.crew_size_actual,src.crew_size_actual,@status_id
	--src.re_run,src.over_run,src.break_use,src.lunch_use
	)
  ...
  --       ELSE  trg.isproductionchanged  END
	...
	WHEN MATCHED  AND isnull(trg.deviation_qty,0) <> isnull(Src.qty,0) THEN -- AND trg.accept_reject IS NULL
	...
	USING (
		select acs.schedule_dt , acs.item_no , acs.variationID , acs.item_nm , acs.facility_cd , acs.qty , f.facility_id
