CREATE PROCEDURE [dbo].[usp_DOR_SCHEDULE_DELETE_ITEM_08272018]
(
...
BEGIN TRY   
	if (@isplaning = 1)
		begin
			declare @changeproduction as int = (select COUNT(*) from RF_DWH_SCHEDULE SC WHERE cast(sc.schedule_date as date) = cast(@schedule_date as date) AND sc.production_modifiedby IS NOT NULL
