CREATE PROCEDURE [dbo].[usp_DOR_GET_SCHEDULE_DETAILS]
...
BEGIN                    
 ...
 ;WITH CTE1 (facility_id,item_id,line_id,line_nm,line_facility_id, item_no,item_nm,line_cd,facility_nm,facility_profit_code)--build_grade_id,crew,t_max)                    
 AS                    
 (                    
...
FROM RF_DWH_ITEM c       
 JOIN RF_DWH_XREF_LINE_FACILITY d ON d.facility_id IN ( SELECT coalesce (@facility_id,facility_id) FROM RF_DWH_FACILITY)  --  Commneted on 06/18/2018 @facility_id                 
 JOIN RF_DWH_FACILITY f ON d.facility_id = f.facility_id 
 ...
 item_nm,line_cd,build_grade_id, crew,t_max,per,qty,                  
  schedule_date,facility_id,seq_no ,line_nm,item_product_variation_id,applecore_variation_id ,comment,isproductionchanged, facility_nm,
facility_profit_code) as                  
  (                  
...
c.facility_profit_code                 
  ...
  WHERE 1 = 1                    
   -- AND c.facility_id = @facility_id          --  Commneted on 06/18/2018 @facility_id           
...
INSERT INTO #schedule (schedule_id, line_id, line_facility_id, item_id,shift_id, shift_cd, shift_nm,                     
 ...
 re_run_units, over_run_units, use_break, use_lunch,crew_size_actual,status_id,status_desc,category_nm,facility_nm,
...
FROM RF_DWH_SCHEDULE a       
   ...
   LEFT JOIN RF_DWH_CATEGORY AS hh ON hh.category_id = f.category_id AND hh.deleted_by IS NULL                 
   WHERE 1 = 1                        
   -- AND c.facility_id = @facility_id              --  Commneted on 06/18/2018 @facility_id                 
    AND CAST(a.schedule_date AS DATE) = CAST(@date AS DATE)                        
    AND a.schedule_id = COALESCE(@schedule_id,a.schedule_id)                        
 ...
 SELECT schedule_id AS schedule_id,line_id AS line_id,line_facility_id AS line_facility_id,item_id AS item_id,shift_id AS shift_id,                     
  ...
  --re_run,over_run,break_use,lunch_use      
  re_run_units,over_run_units,use_break,use_lunch,crew_size_actual  ,status_id,status_desc  ,category_nm,facility_nm,facility_profit_code    
 FROM #schedule                    
...
;WITH CTE3 (line_id, line_cd, line_nm)                    
 ...
 JOIN RF_DWH_LINE b ON a.line_id = b.line_id                    
   WHERE 1 = 1 AND a.facility_id IN (SELECT  COALESCE (@facility_id ,facility_id) FROM RF_DWH_FACILITY) -- @facility_id      Commneted on 06/18/2018 
