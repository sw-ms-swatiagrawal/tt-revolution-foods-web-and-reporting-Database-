CREATE PROCEDURE [dbo].[usp_DOR_GET_UPLOAD_DETAILS_REPORT]          
(          
...
BEGIN          
 ...
 s.status_desc                   
   ...
   WHERE 1 = 1                      
    AND b.facility_id = COALESCE (@facility_id,b.facility_id) AND b.line_id = COALESCE (@line_id,b.line_id)   
 AND CAST(a.schedule_date AS DATE) BETWEEN CAST(@date AS DATE) AND CAST(@end_date as Date)                    
...
THEN h3.build_grade_id                    
  ...
  WHERE x.deleted_by IS NULL AND x.shift_id = g.shift_id                    
  AND x.line_id = c.line_id AND x.item_id = h.item_id                      
  And x.facility_id =  COALESCE (@facility_id,x.facility_id)  AND x.line_id = COALESCE (@line_id,x.line_id)  
  AND x.item_product_variation_id = v1.item_product_variation_id              
  ...
  WHERE a.facility_id =  COALESCE (@facility_id,a.facility_id)  AND b.line_id = COALESCE (@line_id,b.line_id)   
 ...
 -- WHERE b.facility_id =  COALESCE (@facility_id,b.facility_id) AND b.line_id = COALESCE (@line_id,b.line_id)   
