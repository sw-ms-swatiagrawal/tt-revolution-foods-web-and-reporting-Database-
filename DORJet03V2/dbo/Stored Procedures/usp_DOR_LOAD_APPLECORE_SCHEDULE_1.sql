...
CREATE PROCEDURE [dbo].[usp_DOR_LOAD_APPLECORE_SCHEDULE]      
AS      
BEGIN      
 ...
 BEGIN TRY
  ...
  ;WITH CTE (facility_id,shift_id, [item_id], [item_no],qty,[schedule_date],line_cd,build_grade_id,variationID,default_line_id,schedule_id)      
  AS      
  (      
   SELECT b.facility_id AS facility_id           
	 ...
	 OUTER APPLY
	 (
		select max(sc.schedule_id) as schedule_id from RF_DWH_SCHEDULE sc  
		LEFT JOIN RF_DWH_XREF_LINE_FACILITY lf on lf.facility_id = b.facility_id AND sc.line_facility_id = lf.line_facility_id  
		where c.item_id = sc.item_id AND a.schedule_dt = sc.schedule_date 
		AND pv.item_product_variation_id = sc.item_product_variation_id AND sc.inserted_by = 'AppleCore' 
		AND lf.facility_id = b.facility_id AND sc.deleted_date IS NULL AND sc.deleted_by IS NULL
		GROUP BY sc.item_id,sc.item_product_variation_id,sc.schedule_date,lf.facility_id
	 ) as D   
     WHERE 1 = 1      
  ...
  SELECT a.schedule_id AS schedule_id      
    ...
    FROM #schedule a      
	LEFT JOIN RF_DWH_SCHEDULE b on b.schedule_id = a.schedule_id
 --   LEFT JOIN RF_DWH_SCHEDULE b ON a.line_facility_id = b.line_facility_id 	AND 
  ...
  -- And (b.modified_by IS NULL OR b.modified_by = 'AppleCore')  --Commented for deviation button 09/07/2018
